...
│  LocateAnything-3B   │   ("military tank", "armored vehicle", ...)
│  (annotation/)       │   → YOLO-format boxes, no manual labeling
└──────────────────────┘
    │
    ▼
┌──────────────────────┐   NMS dedup, class collapse, train/val split,
│  Dataset build       │   multi-video merge for domain coverage
└──────────────────────┘
    │
    ▼
┌──────────────────────┐   YOLOv8n / YOLO26n, heavy augmentation
│  YOLO training       │   (training/train.py)
└──────────────────────┘
    │
...
python training/train.py \
    ...
    --model yolo26n.pt \         # or yolov8n.pt
...
- **~93 FPS** full pipeline (vs 36 FPS Python) — **2.8× speedup**
