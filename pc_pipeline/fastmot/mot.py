...
from .detector import SSDDetector, YOLODetector, PublicDetector, UltralyticsDetector
from .feature_extractor import FeatureExtractor, NullFeatureExtractor, TorchFeatureExtractor
...
class MOT:
    def __init__(self, size,
        ...
        if ultralytics_detector_cfg is None:
            ultralytics_detector_cfg = SimpleNamespace()
        ...
        elif self.detector_type == DetectorType.ULTRALYTICS:
            self.detector = UltralyticsDetector(self.size, self.class_ids,
                                                **vars(ultralytics_detector_cfg))

        LOGGER.info('Loading feature extractor models...')
        extractors = []
        if not reid_enabled:
            LOGGER.info('ReID disabled — IoU-only tracking (Kalman + KLT)')
            for _ in feature_extractor_cfgs:
                extractors.append(NullFeatureExtractor())
        else:
            for cfg in feature_extractor_cfgs:
                try:
                    extractors.append(FeatureExtractor(**vars(cfg)))
                    LOGGER.info('TRT ReID feature extractor loaded')
                except Exception as e:
                    LOGGER.warning('TRT feature extractor failed (%s); trying PyTorch ReID', e)
                    try:
                        extractors.append(TorchFeatureExtractor())
                        LOGGER.info('PyTorch ReID (MobileNetV3) loaded — appearance matching active')
                    except Exception as e2:
                        LOGGER.warning('PyTorch ReID also failed (%s); falling back to IoU-only', e2)
                        extractors.append(NullFeatureExtractor())
