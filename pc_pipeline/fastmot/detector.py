...
class UltralyticsDetector(Detector):
    """YOLOv8 detector via ultralytics — drop-in replacement for YOLODetector.

    Runs inference synchronously in detect_async and caches results for
    postprocess, conforming to FastMOT's async detector interface.
    """

    def __init__(self, size, class_ids, weights,
                 conf_thresh=0.35, iou_thresh=0.5,
                 max_area=800000, min_aspect_ratio=0.1, imgsz=640, device=None):
        super().__init__(size)
        import torch
        ...
        self._model = _YOLO(str(weights))
        self._cached = np.empty(0, dtype=DET_DTYPE).view(np.recarray)

        # Try to set up GPU-accelerated resize via cupy (Jetson unified memory)
        try:
            import cupy as _cp
            import cupyx.scipy.ndimage as _cpnd
            self._cp = _cp
            self._cpnd = _cpnd
            self._gpu_resize = True
        except ImportError:
            self._gpu_resize = False
    ...
    def _infer(self, frame):
        # Pass raw frame — ultralytics TRT pipeline handles resize/normalize
        # on GPU internally (CUDA letterbox). No CPU pre-resize needed.
        res = self._model.predict(
            frame, conf=self.conf_thresh, iou=self.iou_thresh,
        ...
        for b in res.boxes:
            ...
            if (h / w) < self.min_aspect_ratio:
                continue
            records.append((tlbr, cls_id, float(b.conf[0])))

        # Second-stage greedy NMS — removes any duplicates that survived YOLO's
        # internal NMS (common when conf is very low and boxes slightly offset).
        records.sort(key=lambda r: r[2], reverse=True)  # highest conf first
        keep = []
        for i, (box_i, cls_i, _) in enumerate(records):
            duplicate = False
            for j in keep:
                box_j = records[j][0]
                xi1 = max(box_i[0], box_j[0]); yi1 = max(box_i[1], box_j[1])
                xi2 = min(box_i[2], box_j[2]); yi2 = min(box_i[3], box_j[3])
                inter = max(0, xi2 - xi1) * max(0, yi2 - yi1)
                union = ((box_i[2]-box_i[0])*(box_i[3]-box_i[1]) +
                         (box_j[2]-box_j[0])*(box_j[3]-box_j[1]) - inter)
        ...
        if records:
            return np.array(records, dtype=DET_DTYPE).view(np.recarray)
        return np.empty(0, dtype=DET_DTYPE).view(np.recarray)
